...
export const LiveHome = (props) => {
    ...
    const eventDate = new Date(props.LiveData.eventStart);
    // Mengubah props.LiveData.time menjadi objek Date

    //membuat time start
    const eventTime = (props.LiveData.eventStart);
    const timePart = eventTime.split('T')[1];
    const hours = timePart.split(':')[0];
    const minutes = timePart.split(':')[1];
    //membuat time end
    const endEventTime = (props.LiveData.eventEnd);
    ...
    const handleClick = () => {
        ...
        if (isButtonActive) {
            ...
            window.open(props.LiveData.url, '_blank');
        }
    };
    ...
    return (
                ...
                <div key={props.LiveData.id} class="mb-3 col-8 col-md-6 col-lg-3">
                    <div class="card h-100 position-relative" data-toggle="tooltip" data-placement="top" title={props.LiveData.name}>
                        <img class="card-img-top" src={props.LiveData.image} alt="cover"></img>
